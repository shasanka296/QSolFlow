...
class lig:
    ...
    def __init__(self, smiles, regular_name, molecule, charge, di, own, own_path):
        ...
        if own:
            ...
            moving_pdb_command = (
                f"mv {self.path_to_pdb} {os.path.join(self.dir, self.mol)}"
            )
    ...
    def PDBMAKER(self, name, smiles):
        if name == "MET":
            name = "MeT"
        with open(
            f"{os.path.join(self.dir, self.mol, f'{self.mol}.pdb')}", "r"
        ) as fie, open(f"{os.path.join(self.dir, self.mol, 'new.pdb')}", "a") as new:
            f = fie.readlines()
            line_to_print = []
            for iteams in f:
