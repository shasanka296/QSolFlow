...
class reorg:
    ...
    def __init__(self, molecule, direc, key):
        ...
        subprocess.run(
            f"touch {os.path.join(self.dir, f'{self.moleclue}.itp')}", shell=True
        )
        if not os.path.isfile(os.path.join(self.dir, f"{self.moleclue}_atomtype.itp")):
            ...
            subprocess.run(
                f'touch {os.path.join(self.dir, f"{self.moleclue}_atomtype.itp")}',
                shell=True,
            )
    ...
    def write_type(self):
        with open(
            f'{os.path.join(self.dir, f"{self.moleclue}_atomtype.itp")}', "a"
        ) as a_type:
            a_type.writelines("\n")
