...
class MDPrep(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        if self.initial_system == False:
            ...
            print(names)
            i = 0
            for iteams, name, mul, charge in zip(
                smiles, names, self.multipicity, self.charge
            ):
                if i >= 1:
                    transfer.trans(
                        f"{name[:3]}_Solute1",
...
class TitrationChargeScaler(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        self.titration_matrix = fw_spec.get("titration_list") or self.get(
            "titration_list"
        )
...
class Density(FiretaskBase):  ### need to fix this block later
    ...
    def run_task(self, fw_spec):
        ...
        subprocess.run(
            [f" touch {os.path.join(meta_dir, f'InputGrofiles{density_key}', 'data')}"],
            shell=True,
        )
        print(f"{os.path.join(meta_dir, f'InputGrofiles{density_key}', 'data')}")
        with open(
            f"{os.path.join(meta_dir, f'InputGrofiles{density_key}', 'data')}", "a"
        ) as file:
        ...
        return FWAction(
            update_spec={
                f"Average_den{density_key}": density_mat,
                "outputden": output_density,
            }
        )
...
class key_gen(FiretaskBase):
    ...
    def organize(self, key, date_submitted, dirs):
        ...
        subprocess.run(
            [f"mv {os.path.join(dir, 'key')} {run_dir}"], shell=True, check=True
        )
        for i, j in zip(self.names, self.type):
            repeat = 0
            try:
                subprocess.run(
                    [f"rm -rf {os.path.join(dir, f'{i[:3]}_{j}')}"],
                    shell=True,
                    check=True,
                )
        ...
        for iteam in key:
            subprocess.run(
                [f"mv {os.path.join(dir, f'InputGrofiles{key[iteam]}')} {run_dir}"],
                shell=True,
            )
            subprocess.run(
                [f"mv {os.path.join(dir, f'Output{key[iteam]}')} {run_dir}"], shell=True
            )
...
class Graph_plotter(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        Average_simulation_density_dic = {
            i: j for i, j in zip(titration_list, Average_simulation_density)
        }
